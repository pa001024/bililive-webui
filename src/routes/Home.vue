...
<template>
  <div class="main-container">
    <div class="queue">
      <div class="header" v-if="requests.length">
        当前队列 ({{ requests.length }}/4)
      </div>
      <div class="header" v-else>当前队列为空</div>
      ...
      <transition-group name="fade">
        <Queue
          ...
          :req="req"
          :key="req.id"
          :active="req.id === activeReq"
        />
      </transition-group>
    </div>
    <div class="alert" v-if="alertText">{{ alertText }}</div>
